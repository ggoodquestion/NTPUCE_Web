...
<body class="">
    ...
    <div id="wrapper" class="fade-in">
        ...
        $sql = "SELECT COUNT(id) FROM article WHERE category='activity';";
        $result = mysql_query($sql);
        $row = mysql_fetch_row($result);
        $totalPage = ceil($row[0] / 6);

        $sql = "SELECT * FROM article WHERE category='activity' ORDER BY date desc LIMIT $first, $numPerPage;";
        $result = mysql_query($sql);
        ...
        <!-- Main -->
        <div id="main">
            <?php listPostForOnePage("學期活動", $postList, $totalPage, $currentPage, "./activity.php", "page"); ?>
        </div>
